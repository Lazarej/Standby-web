...
export default function HomeCarousel() {
  ...
  const Transition = async(index) =>{
    await tl.to('#title',{left:'-50%' , duration:0.5});
    setIndex( prev => prev = index) 
    tl.to('#title',{left:0, duration:0.5});
  ...
  return (
    <div className="w-full flex flex-col mb-10 md:flex-row md:border-t md:border-lightGrey md:w-[90%] relative">
      ...
      <div className="h-14 w-full flex  border-b border-lightGrey md:w-[14vw] md:h-auto md:flex-col  md:border-b-0 ">
        ...
        <div className="grow max-h-96 border-l  border-l-lightGrey flex justify-around md:h-[400px] md:flex-col md:border-t md:border-lightGrey md:border-l-0 md:items-center">
          <BiPaperPlane onClick={ ()=> Transition(0)} size={40} color={index === 0 ? fullConfig.theme.colors.primary : 'black'} />
          <IoNewspaperOutline onClick={  ()=> Transition(1)} size={40} color={index === 1 ? fullConfig.theme.colors.primary : 'black'}/>
          <MdOutlineFavoriteBorder onClick={ ()=>  Transition(2)} size={40} color={index === 2 ? fullConfig.theme.colors.primary : 'black'}/>
        </div>
      </div>
      <div className="w-full min-h-[400px] flex">
        ...
        <div className="grow border-l md:border-l-0 border-l-lightGrey relative pt-32 md:pt-36">
          <div className="md:relative md:top-[-6vw] md:min-w-36  md:overflow-hidden md:h-16 md:left-[-10vw]">
          <h3 id="title" className="absolute top-12 left-[-50px] font-robotoCB text-primary uppercase text-4xl tracking-tighter md:text-[4vw] md:top-1 md:left-0">/ / {array[index].title}</h3>
          </div>
